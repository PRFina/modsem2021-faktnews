...
REPO_CONFIG = {
  ...
  "params": {
    ...
    "baseURL": {
      "label": "Base URL",
      "name": "baseURL",
      "value": os.environ['RDF_STORE_BASE_URL'], 
    },
...
if __name__ == '__main__':
  ...
  resp = requests.post("http://rdf:7200/rest/repositories",
  ...
  ontology_path = Path("rdf/fakt-news-materialized-triples.rdf")
  ontology_file = ontology_path.open("rb")

  URL = "http://rdf:7200/repositories/{}/statements".format(REPO_CONFIG['id'])
  resp = requests.post(URL, data=ontology_file, headers={'Content-Type': 'application/rdf+xml'})

  if resp.status_code == requests.codes['no_content']:
      logging.info("imported {} into {}".format(ontology_path,REPO_CONFIG['id']))
  else:
      logging.warning("Status code {}".format(resp.status_code))
      logging.warning("Response message {}".format(resp.text))
